...
select departamento.dnome,max(salario),min(salario) from empregado inner join departamento on departamento.codigo = empregado.cdep group by dnome
-- 2. Recuperar o nome do departamento com maior média salarial.
select departamento.dnome from empregado inner join departamento on departamento.codigo = empregado.cdep group by dnome order by avg(salario) desc limit 1
...
select projeto.pnome from projeto inner join tarefa on tarefa.pcodigo = projeto.pcodigo group by projeto.pnome order by sum(horas) desc limit 1
-- 5. Recuperar o nome do projeto mais caro.
select projeto.pnome from projeto inner join tarefa on tarefa.pcodigo = projeto.pcodigo inner join empregado on empregado.cpf = tarefa.cpf group by projeto.pnome order by sum(empregado.salario) desc limit 1
...
select enome from empregado where cpf in (select gerente from departamento) and enome ~ '^.+\sSilva'
...
select empregado.enome from empregado inner join (select empregado.cpf,count(distinct tarefa.pcodigo) from empregado inner join tarefa on tarefa.cpf = empregado.cpf group by empregado.cpf) as "projetos" on "projetos".cpf = empregado.cpf and "projetos".count = (select count(*) from projeto)
...
select * from empregado where salario between 700 and 2800
